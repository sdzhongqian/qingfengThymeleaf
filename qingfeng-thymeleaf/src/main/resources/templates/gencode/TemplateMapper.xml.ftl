...
<#if tablePd.temp_type == '1'>
    ...
    <!--查询子表列表-->
    <select id="findChildList" parameterType="com.qingfeng.util.PageData" resultType="com.qingfeng.util.PageData">
        select
	<#list linkFieldList as obj>
		...
		<#if obj.field_query == 'order_by'>
			<#assign ownChildOrderBy = 'true'>
		</#if>
	</#list>
			from ${linkTablePd.link_table_name} a
			where a.${linkTablePd.link_field}=${'#'}{id}
		<#if ownChildOrderBy == 'true'>
			order by a.order_by+0 asc , a.create_time desc
		</#if>
		<#if ownChildOrderBy == 'false'>
			order by a.create_time desc
		</#if>
    </select>

    <!--保存子表-->
    <insert id="saveChild" parameterType="com.qingfeng.util.PageData">
        insert into ${linkTablePd.link_table_name} (
		<#list linkFieldList as obj>
			<#if obj_has_next>
			${obj.field_name},
			</#if>
    ...
    <!--更新子表-->
    <update id="updateChild" parameterType="com.qingfeng.util.PageData">
        update ${linkTablePd.link_table_name} set
	<#list linkFieldList as obj>
		<#if obj.field_name != 'update_time'&&obj.field_name != 'create_user'&&obj.field_name != 'create_organize'&&obj.field_name != 'create_time'>
			<if test="${obj.field_name} != null">${obj.field_name}=${'#'}{${obj.field_name}},</if>
		</#if>
    ...
    <!--删除子表-->
    <delete id="delChild" parameterType="String">
        delete from ${linkTablePd.link_table_name} where id in
        <foreach collection="array" item="id" open="(" separator="," close=")">
		${'#'}{id}
        </foreach>
    </delete>
    ...
    <!--删除子表-->
    <delete id="delChildForPIds" parameterType="String">
        delete from ${linkTablePd.link_table_name} where ${linkTablePd.link_field} in
        <foreach collection="array" item="id" open="(" separator="," close=")">
		${'#'}{id}
        </foreach>
    </delete>

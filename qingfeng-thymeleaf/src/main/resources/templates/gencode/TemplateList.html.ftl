                ...
                <#list fieldList as obj>
                    <#if obj.field_query == 'Y'>
                        ...
                        <#if obj.show_type == '5'>
                            ...
                            <div class="layui-inline">
                            <#if tablePd.temp_type == '2'>
                                <button type="button" class="layui-btn layui-btn-sm"  lay-submit="" th:onclick="reload()"><i class="layui-icon">&#xe615;</i></button>
                            </#if>
                            <#if tablePd.temp_type != '2'>
...
<th:block th:include="web/${tablePd.mod_name}/${tablePd.bus_name}/${tablePd.bus_name}_ztree"></th:block>
</#if>
...
<script>
    ...
    layui.use(['table','form'], function(){
        ...
        table.render({
            ...
            ,url:ctx+'${tablePd.mod_name}/${tablePd.bus_name}/findListPage'
    ...
    <#if obj.field_query == 'Y'>
            ...
            switch(obj.event){
                ...
                case 'del': //删除
                    if(data.length>0){
                        ...
                        ${'$'}.get(ctx+"${tablePd.mod_name}/${tablePd.bus_name}/del?ids="+ids+"&names="+names,null,function(res){
                                if (res.success) {
        ...
        table.on('tool(${tablePd.mod_name}_${tablePd.bus_name})', function(obj){
            ...
            if(obj.event === 'info'){
                ...
                layer.confirm('真的要删除数据么?', function(index){
                ${'$'}.get(ctx+"${tablePd.mod_name}/${tablePd.bus_name}/del?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                        <#if tablePd.temp_type == '2'>
                            reload();
...
//            layer.msg('[ID: '+ data.id +'] ' + field + ' 字段更改为：'+ value);
            $.ajax({
                ...
                url: ctx+"${tablePd.mod_name}/${tablePd.bus_name}/update?"+field+"="+value ,//url
                data: {
                    id:data.id
                },
    ...
    function add(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                type: 2,
                <#if tablePd.temp_type == '2'>
                    content: ctx+'${tablePd.mod_name}/${tablePd.bus_name}/toAdd?${tablePd.tree_pid}='+parent_id+'&${tablePd.tree_name}='+name,
                </#if>
                <#if tablePd.temp_type != '2'>
                    content: ctx+'${tablePd.mod_name}/${tablePd.bus_name}/toAdd',
                </#if>
                area: ['800px', '500px'],
                end: function () {
...
<#if tablePd.more_add == '1'>
    ...
    function addMore(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                type: 2,
                <#if tablePd.temp_type == '2'>
                    content: ctx+'${tablePd.mod_name}/${tablePd.bus_name}/toAddMore?type=${'$'}{pd.type}&${tablePd.tree_pid}='+parent_id+'&${tablePd.tree_name}='+name,
                </#if>
                <#if tablePd.temp_type != '2'>
                    content: ctx+'${tablePd.mod_name}/${tablePd.bus_name}/toAddMore?type=${'$'}{pd.type}',
                </#if>
                area: ['800px', '500px'],
                end: function () {
                    var val = getOpenCloseParam();
    ...
    function edit(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: ctx+'${tablePd.mod_name}/${tablePd.bus_name}/toUpdate?id='+id,
            area: ['800px', '500px'],
            end: function () {
                var val = getOpenCloseParam();
                if(val=="reload"){
    ...
    function info(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: ctx+'${tablePd.mod_name}/${tablePd.bus_name}/findInfo?id='+id,
            area: ['800px', '500px']
        });
    }

    function updateStatus(id,status){
        $.get(ctx+"${tablePd.mod_name}/${tablePd.bus_name}/updateStatus?id="+id+"&status="+status,null,function(res){
            if (res.success) {
                layer.tips("状态修改成功。");
            <#if tablePd.temp_type == '2'>
                reload();

<script type="text/javascript">
    ...
    function findTreeData(tree_id){
        var treeNodes = '';
        $.ajax({
            url : ctx+"${tablePd.mod_name}/${tablePd.bus_name}/findList",
            data : {},
            type : 'GET',
            dataType : 'json',
            success : function(res) {
                if (res.success) {
                    treeNodes += "[{id:'0',parent_id:'parent',name:'${tablePd.menu_name}'},";
                    $.each(res.data,function(i,n){
                        treeNodes += "{";
                        treeNodes += "id:'"+n.id+"'";
                        treeNodes += ",parent_id:'"+n.${tablePd.tree_pid}+"'";
                        treeNodes += ",name:'"+n.${tablePd.tree_name}+"'";
                        treeNodes += "},";
                    });

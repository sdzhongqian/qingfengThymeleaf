...
function uploadMoreFile(index,type){
    ...
    upload.render({
        ...
        ,url: ctx+'common/upload/uploadFile?source='+type
        ,multiple: true
        ,accept: 'file' //普通文件
        ,done: function(res){
...
function delMoreFile(index,id,file_path){
    layer.confirm('删除文件后，只能重新上传，真的要删除文件么？', function(i){
        ...
        $.ajax({
            ...
            url: ctx+"common/upload/delFile?id="+id+"&file_path="+file_path,//url
            data: "",
...
function downloadFile(id,file_path,name){
    window.location.href=ctx+"common/upload/downloadFile?name="+name+"&file_path="+file_path;
}

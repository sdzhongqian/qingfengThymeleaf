...
function findAddress(parent_id,show_id,value){
    $.ajax({
        ...
        url: ctx+"system/area/findList?parent_id="+parent_id,//url
        data: "",
...
function findSelectDictionary(parent_id,show_id,value){
    $.ajax({
        type: "GET",//方法类型
        dataType: "json",//预期服务器返回的数据类型
        url: ctx+"system/dictionary/findList?parent_id="+parent_id,//url
        data: "",
        success: function (res) {
            if (res.success) {
                var tt = '<option value="">请选择</option>';
...
//查询Checkbox字典项
function findCheckboxDictionary(parent_code,show_ctx,show_id,value){
    $.ajax({
        type: "GET",//方法类型
        dataType: "json",//预期服务器返回的数据类型
        url: ctx+"system/dictionary/findList?parent_code="+parent_code,//url
        data: "",
        success: function (res) {
            if (res.success) {
                var tt = '';
                $.each(res.data,function(i,n){
                    if(value.indexOf(n.id)!=-1){
...
function findValueDictionary(show_id,value){
    $.ajax({
        type: "GET",//方法类型
        dataType: "json",//预期服务器返回的数据类型
        url: ctx+"system/dictionary/findInfoJson?id="+value,//url
        data: "",
        success: function (res) {
            if (res.success) {
                var names = [];
...
function findTabClassify(show_id,value){
    $.ajax({
        type: "GET",//方法类型
        dataType: "json",//预期服务器返回的数据类型
        url: ctx+"customize/tclassify/findList?status=Y",//url
        data: "",
        success: function (res) {
            if (res.success) {
                var tt = '<option value="">请选择</option>';
...
function findTableList(classify_id,show_id,value){
    $.ajax({
        type: "GET",//方法类型
        dataType: "json",//预期服务器返回的数据类型
        url: ctx+"customize/ttable/findList?status=Y&classify_id="+classify_id,//url
        data: "",
        success: function (res) {
            if (res.success) {
                var tt = '<option value="">请选择</option>';
